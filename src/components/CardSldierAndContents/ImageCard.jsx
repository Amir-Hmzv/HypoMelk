  ...
  return (
    <div className="relative">
        ...
        {item.media.map((image, i) => {
          return (
            <SwiperSlide key={i} className="re">
              <ImageCardContent item={item} image={image} />
            </SwiperSlide>
          );
        })}
      </Swiper>
      <div className=" inset  relative  z-20 ">
        <div
          onClick={handleNext}
          className={`prev absolute bottom-[89px] left-3  rounded-full bg-white p-[3px] drop-shadow-[0_0_15px_rgba(0,0,0,0.25)] ${
            isLast && "opacity-50"
          }`}
        >
        ...
        <div
          onClick={handlePrev}
          className={`next absolute bottom-[89px] right-3 rounded-full bg-white p-[3px] drop-shadow-[0_0_15px_rgba(0,0,0,0.25)] ${
            isFirst && "opacity-50"
          }`}
        >
        ...
        <div className="absolute bottom-[149px] right-3 flex items-center  ">
          <div
            className={`flex flex-row-reverse whitespace-nowrap rounded-lg bg-special p-1 font-IRANYekanWeb lg:p-2 ${
              !item?.special && "hidden"
            } `}
          >
            <span className={`mx-1 font-IRANYekanWeb ${style.specialText} `}>
              فروش ویژه
            </span>
            <span> {specialIcon}</span>
          </div>
          <div
            className={`mr-2 ${style.presell} ${
              !item.Presell && "hidden"
            } whitespace-nowrap rounded-lg bg-precell  p-1 lg:p-2`}
          >
            پیش فروش
          </div>
        </div>
